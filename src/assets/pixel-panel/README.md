...
| `bottom-ground.png` | 五种状态共用的全宽草地、落花与小花基线；672 × 36，按 2× 显示密度输出 |
...
- 底部装饰采用双层结构：`bottom-ground.png` 负责贯穿全宽的低矮地景，各状态 `bottom-*.png` 负责保持比例的树木与角色场景。
...
- 状态场景保留较宽透明内容区，由 Panel 中的独立全宽地面层补齐左右边缘，禁止直接拉伸角色场景。
